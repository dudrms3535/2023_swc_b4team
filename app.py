...
data = requests.get(
    'https://comic.naver.com/webtoon/finish?order=ViewCount&view=image', headers=headers)
...
movies = soup.select('#content > div.list_area > ul > li')


# movies (tr들) 의 반복문을 돌리기
for movie in movies:
# movie 안에 a 가 있으면,(타이틀수정)
    a_tag = movie.select_one('dl > dt > a')
    if a_tag is not None:
        title = a_tag.text  # a 태그 사이의 텍스트를 가져오기(타이틀가져오기)
        doc = {
            'title': title,
        }
